...
import { Download, Music, MousePointerClick, Star, RefreshCw, Users } from "lucide-react";
...
function exportCSV(logs: LogEntry[]) {
  ...
  const rows = logs.map((d) => [d.timestamp, new Date(d.timestamp).toISOString(), d.owner_email || "", d.button, d.soundfile].join(","));
...
export default function Research() {
  ...
  const dailyPresses = useMemo(() => {
    const counts = new Map<string, number>();
    logs.forEach((l) => {
      const day = new Date(l.timestamp).toLocaleDateString(undefined, { weekday: "short" });
  ...
  const hourlyPattern = useMemo(() => {
    const counts = new Map<string, number>();
    logs.forEach((l) => {
      const hour = new Date(l.timestamp).toLocaleTimeString(undefined, { hour: "2-digit", hour12: false });
  ...
  const ownerStats = useMemo(() => {
    return profiles.map((profile) => {
      const ownerLogs = logs.filter((log) => profile.email && log.owner_email === profile.email);
      const favorite = ownerLogs.reduce<Record<string, number>>((acc, log) => {
        const key = log.soundfile || "Unassigned";
        acc[key] = (acc[key] || 0) + 1;
        return acc;
      }, {});
      const favoriteSound = Object.entries(favorite).sort((a, b) => b[1] - a[1])[0]?.[0] || "No data yet";
  ...
  const activityStats = [
    { label: "Registered Owners", value: String(profiles.length), icon: Users },
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Overview of registered owners and button-press data.</p>
        </div>
      ...
      <Card>
        ...
        <CardContent>
          {ownerStats.length === 0 ? (
            <p className="text-sm text-muted-foreground">No owners registered yet.</p>
          ) : (
                ...
                <tbody>
                  {ownerStats.map((owner) => (
                    <tr key={owner.email || owner.id} className="border-b last:border-0">
                      ...
                      <td className="py-2 pr-4">{owner.agreement_accepted ? "Accepted" : "Not accepted"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {logs.length === 0 && (
        <Card>
          <CardContent className="pt-6 text-sm text-muted-foreground">No button presses logged yet. Go to Device Configuration and click a device button, or connect the Arduino and press a real button.</CardContent>
        </Card>
      )}
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {logs.slice(-10).reverse().map((log, index) => (
                    <tr key={`${log.timestamp}-${index}`} className="border-b last:border-0">
                      ...
                      <td className="py-2 pr-4">{log.soundfile || "Unassigned"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
